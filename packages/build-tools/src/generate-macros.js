...
function generateButtonMacro() {
  ...
  {# Uses BEM classes from components.css which leverage design tokens #}
  {# This ensures button styling works in HubSpot without Tailwind #}
  ...
  {% if interactive %}
    ...
    <div class="react-island"
      ...
      {# Server-rendered fallback using BEM classes with design tokens #}
  ...
  {% else %}
    {# Static mode - server-rendered only, using BEM classes with design tokens #}
    <a href="{{ href }}"
...
function generateCardMacros() {
  ...
  {# Uses BEM classes from react-components.css which leverage design tokens #}
...
function generateHeroMacro() {
  ...
  <section class="huble-hero">
    <div class="huble-hero__container">
      <div class="huble-hero__grid--with-image">
        {# Content section - order changes based on image position #}
        <div class="huble-hero__content"
             style="order: {% if imagePosition == 'left' %}2{% else %}1{% endif %}">
          <h1 class="huble-hero__heading">
            {{ heading }}
          </h1>
          {% if description %}
            <p class="huble-hero__description">
              {{ description }}
            </p>
          {% endif %}
          {# CTA buttons area #}
        ...
        {# Image section #}
        {% if imageSrc %}
          <div style="order: {% if imagePosition == 'left' %}1{% else %}2{% endif %}">
            <img src="{{ imageSrc }}"
                 alt="{{ imageAlt }}"
                 class="huble-hero__image"
                 width="550"
                 height="310"
                 loading="lazy">
          </div>
